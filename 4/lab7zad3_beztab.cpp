...
double *min_locals;
double global_min;
#pragma omp parallel shared(nthreads, min_locals, n_points, tab)
{
int id = omp_get_thread_num();
#pragma omp single
{
nthreads = omp_get_num_threads();
min_locals = new double[nthreads];
printf("Uruchomiono watkow: %d\n", nthreads);
}
long ppt = (long)ceil((double)n_points / nthreads);
long sp = id * ppt;
long current_ppt = ppt;
if (sp + ppt > n_points) current_ppt = n_points - sp;
if (sp >= n_points) current_ppt = 0;
if (current_ppt > 0) {
min_sub_tab(tab, sp, current_ppt, &min_locals[id]);
}
}
min_sub_tab(min_locals, 0, nthreads, &global_min);
*tab_min = global_min;
delete[] min_locals;
}
int main() {
